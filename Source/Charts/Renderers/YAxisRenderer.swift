...
open class YAxisRenderer: AxisRendererBase
{
    ...
    open func renderLimitLines_byWinston(context: CGContext, image: UIImage, size: CGSize)
    {
        guard
            let yAxis = self.axis as? YAxis,
            let viewPortHandler = self.viewPortHandler,
        ...
        for i in 0 ..< limitLines.count
        {
            ...
            let label = l.label
            /* winston test */
            ChartUtils.drawImage(context: context, image: image, x: viewPortHandler.contentLeft + size.width/2, y:  position.y, size: size)
